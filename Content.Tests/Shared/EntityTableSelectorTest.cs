...
public sealed partial class EntityTableSelectorTest : ContentUnitTest
{
    ...
    public void EntSelectorTest()
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(_entTable.ListSpawns(s), Is.EquivalentTo([(a: A, 1f)]));
            Assert.That(_entTable.AverageSpawns(s), Is.EquivalentTo([(a: A, 6f)]));
        }
    }
    ...
    [Test]
    public void GroupSelectorWithoutDuplicatesTest()
    {
        // Four rolls with two children, so we should always get the same results.
        var s = new GroupSelector
        {
            Prob = 1f,
            Rolls = new ConstantNumberSelector(2),
            CanRollDuplicates = false,
            Children =
            [
                new EntSelector { Id = A, Weight = 3f },
                new EntSelector { Id = B, Weight = 1f },
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.That(_entTable.GetSpawns(s, Rand()), Is.EquivalentTo([A, B]));
            Assert.That(_entTable.ListSpawns(s), Is.EquivalentTo([(A, 1f), (B, 1f)]));
            Assert.That(_entTable.AverageSpawns(s), Is.EquivalentTo([(A, 1f), (B, 1f)]));
        }
    }
...
";
    ...
    public void NestedSelectorTest()
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(_entTable.ListSpawns(s), Is.EquivalentTo([(a: A, 1f)]));
            Assert.That(_entTable.AverageSpawns(s), Is.EquivalentTo([(a: A, 1f)]));
        }
    }
