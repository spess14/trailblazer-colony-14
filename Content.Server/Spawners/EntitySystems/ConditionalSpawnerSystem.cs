...
public sealed partial class ConditionalSpawnerSystem : EntitySystem
{
    ...
    private void Spawn(Entity<EntityTableSpawnerComponent> ent)
    {
        ...
        if (comp.AutoStack)
        {
            SpawnStackedWhenPossible(spawns, ent, coords, comp.Offset, rotation, ent.Comp.RandomRotation); // Moff - Random rotation
        }
        else
        {
            SpawnAtRandomOffset(spawns, coords, comp.Offset, rotation, ent.Comp.RandomRotation); // Moff - Random rotation
        }
    }
    ...
    private void SpawnStackedWhenPossible(IEnumerable<EntProtoId> spawns,
        ...
        float offset,
        Angle rotation, // Moff - Random rotation
        bool randomRotation) // Moff - Random rotation
    {
        ...
        SpawnAtRandomOffset(nonStackable, coords, offset, rotation, randomRotation); // Moff - Random rotation
    ...
    private void SpawnAtRandomOffset(IEnumerable<EntProtoId> spawns, MapCoordinates coords, float offset, Angle rotation, bool randomRotation) // Moff - Random rotation
    {
        foreach (var proto in spawns)
        {
            SpawnAtRandomOffset(proto, coords, offset, rotation, randomRotation); // Moff - Random rotation
        }
    }

    private EntityUid SpawnAtRandomOffset(EntProtoId proto, MapCoordinates coords, float offset, Angle rotation, bool randomRotation) // Moff - Random rotation
    {
        var trueCoords = GetRandomOffset(coords, offset);

        return Spawn(proto, trueCoords, rotation: randomRotation ? _robustRandom.NextAngle() : rotation); // Moff - Random rotation
    }
